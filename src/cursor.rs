...
impl<'a, Qs> Cursor<'a, Qs> {
    ...
    pub fn key<T>(&self) -> crate::Result<T>
    where
        T: TryFromJs,
    {
        T::from_js(self.as_sys().key()?).map_err(Into::into)
    }

    /// [`Self::key`] mirror for `serde`-deserialisable keys.
    #[cfg(feature = "serde")]
    #[allow(clippy::missing_errors_doc)]
    pub fn key_ser<T>(&self) -> crate::Result<T>
    where
        T: crate::serde::DeserialiseFromJs,
    {
        T::deserialise_from_js(self.as_sys().key()?)
    }
...
impl<'a, Qs> SystemRepr for Cursor<'a, Qs> {
