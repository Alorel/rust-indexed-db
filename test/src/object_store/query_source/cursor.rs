pub mod key_cursor {
    ...
    pub mod base_cursor {
        ...
        pub async fn primary_key() {
            ...
            assert_eq!(primary_key!(Key, cursor), Ok(Key::MIN + 1));
        }
        ...
        pub async fn advance_by() {
            ...
            assert_eq!(pk1, Ok(Key::MIN + 5));
            assert_eq!(key1, Ok(Some(Key::MIN + 5)));
            assert_eq!(pk2, Ok(Key::MIN + 5));
            assert_eq!(key2, Ok(Some(Key::MIN + 6)));
            assert_eq!(pk3, Ok(Key::MIN + 6));
        ...
        pub async fn continue_to_key() {
            ...
            assert_eq!(pk, Ok(Key::new(-5)));
            assert_eq!(key, Ok(Some(Key::new(-5))));
            assert_eq!(pk2, Ok(Key::new(-5)));
        }
    }
    ...
    pub mod stream {
        ...
        pub async fn delete() {
            async fn mutate(db: &Database) {
                ...
                assert_eq!(primary_key!(Key, stream), Ok(Key::MIN));
                assert_eq!(
                    stream.next().await,
                    Some(Ok(KeyVal::new(Key::MIN + 1, Value::MAX - 1)))
                );
                assert_eq!(primary_key!(Key, stream), Ok(Key::MIN + 1));
